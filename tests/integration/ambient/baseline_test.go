...
func getIngressGatewayServiceAccount(t framework.TestContext, gatewayNamespace, gatewayName string) string {
	cluster := t.Clusters().Default()
	appsClient := cluster.Kube().AppsV1()

	// Get the ingress gateway deployment
	dep, err := appsClient.Deployments(gatewayNamespace).Get(
		context.TODO(),
		gatewayName,
		metav1.GetOptions{},
	)
	if err != nil {
		t.Fatalf("Failed to get ingress gateway deployment %s/%s: %v", gatewayNamespace, gatewayName, err)
	}
...
func TestAuthorizationGateway(t *testing.T) {
	...
	framework.NewTest(t).Run(func(t framework.TestContext) {
		...
		ingressGatewaySa := getIngressGatewayServiceAccount(t, ingressGatewayNs, ingressGatewaySvc)
