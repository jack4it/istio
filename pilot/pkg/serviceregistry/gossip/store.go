...
var storeLog = log.RegisterScope("gossip-store", "Gossip data store")

// gossipStore is an internal data store for gossip-synced services.
...
// gossipStoreConfig contains configuration for the gossip store.
...
// newGossipStore creates a new gossip data store.
func newGossipStore(cfg gossipStoreConfig) *gossipStore {
	return &gossipStore{
...
func (s *gossipStore) handleSyncMessage(msg *SyncMessage) {
	if msg == nil {
		return
	}
...
// getGossipState returns all services and workloads from the gossip store.
// This is used to populate KRT collections for ambient index integration.
func (s *gossipStore) getGossipState() ([]model.ServiceInfo, []model.WorkloadInfo) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *gossipStore) createSplitHorizonWorkload(svcInfo *model.ServiceInfo, gateway model.NetworkGateway) *model.WorkloadInfo {
...
func (s *gossipStore) createNetworkGatewayWorkload(wireGw *WireNetworkGateway) *model.WorkloadInfo {
	if wireGw == nil {
		return nil
	}
...
func (s *gossipStore) addLocalNetworkVIPs(svc *model.ServiceInfo, localNetwork string) *model.ServiceInfo {
